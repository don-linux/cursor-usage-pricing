...
### Font embed
`@font-face` loading from `../.cursor/skills/cursor-brand/assets/fonts/WEB/WOFF2/*.woff2` works inside Chrome headless via `file://` — confirmed. Set `font-display: block` so the screenshot waits on the font.
...
3. If the user is working inside an installed copy of this skill (e.g. `~/.cursor/skills/cursor-brand/`), also offer to open a PR against [`cursorcommunityled/cursor-brand`](https://github.com/cursorcommunityled/cursor-brand) so the learning flows back to the community.
